...
func (_q *MessageQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*Message, init func(*Message), assign func(*Message, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Message)
	for i := range nodes {
		if nodes[i].AuthorUserID == nil {
			continue
		}
...
func (_q *MessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withAuthor != nil {
			_spec.Node.AddColumnOnce(message.FieldAuthorUserID)
		}
	}
