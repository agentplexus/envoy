...
func (_c *IdentityCreate) check() error {
	...
	if _, ok := _c.mutation.ProviderSubject(); !ok {
		return &ValidationError{Name: "provider_subject", err: errors.New(`ent: missing required field "Identity.provider_subject"`)}
	}
	...
	if _, ok := _c.mutation.VerifiedEmail(); !ok {
		return &ValidationError{Name: "verified_email", err: errors.New(`ent: missing required field "Identity.verified_email"`)}
	}
