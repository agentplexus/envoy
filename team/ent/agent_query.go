...
func (_q *AgentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Agent, error) {
	...
	if query := _q.withRoles; query != nil {
		if err := _q.loadRoles(ctx, query, nodes,
			func(n *Agent) { n.Edges.Roles = []*AgentRole{} },
			func(n *Agent, e *AgentRole) { n.Edges.Roles = append(n.Edges.Roles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AgentQuery) loadSkills(ctx context.Context, query *AgentSkillQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *AgentSkill)) error {
	...
	query.Where(predicate.AgentSkill(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(agent.SkillsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *AgentQuery) loadRoles(ctx context.Context, query *AgentRoleQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *AgentRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Agent)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(agentrole.FieldAgentID)
	}
	query.Where(predicate.AgentRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(agent.RolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
