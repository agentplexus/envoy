...
- [ ] `RMI-OMNIAGENT-200` Secret declaration in SKILL.md frontmatter
...
- [ ] `RMI-OMNIAGENT-205` `user_secrets` schema + RLS policies
  - Depends on: `RMI-OMNIAGENT-003` initiative RLS store (`RMI-OMNIAGENT-101`)
  - Acceptance: owner-only RLS (superadmin not content-privileged); `(user_id, skill, name)` unique; migration idempotent
- [ ] `RMI-OMNIAGENT-206` Envelope encryption (AES-256-GCM + omnivault KEK)
  - Acceptance: KEK resolved from `team.secrets.encryption_key`, never stored; values sealed with per-write nonce + name/skill AAD; DB dump yields no plaintext
- [ ] `RMI-OMNIAGENT-207` Per-user secret service (write-only values)
...
- [ ] `RMI-OMNIAGENT-211` Secrets HTTP API
  - Depends on: `RMI-OMNIAGENT-207`
...
- [ ] `RMI-OMNIAGENT-212` Settings › Secrets UI panel
